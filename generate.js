...
const inputpath = "./srv_wsdl"
const outputpath = "./services/stubs"
...
srv_wsdl.forEach((element) => {
  ...
  var generateMethod = function(name, method, indent) {
    ...
    return util.format([indent, "//", "%s", lineEnd,
      ...
      (throwFauls ? [indent, tab, "throw NOT_IMPLEMENTED;", lineEnd].join('') :
        ""),
  ...
  var generateParameter = function(name, object, indent, depth) {
    ...
    var r = util.format(
      ["var %s = { %s};"].join(''),
      name,
      (parameter.length > 0 ? [parameter.join("," + lineEnd), lineEnd, indent].join('') :
        "")
    );
    return r;
  };
  ...
  var generateType = function(object, indent, depth) {
    ...
    while (child = object.children[c++]) {
      ...
      } else if (child.name == "anyAttribute") {} else if (child.name == "annotation") {} else if (child.name == "extension") {} else if (child.name == "simpleContent") {
    ...
    var r = util.format(
      ["%s%s"].join(''),
      (attr.length ? [lineEnd, indent, "attributes : {", lineEnd, attr.join(',' + lineEnd), lineEnd, indent, "}", (props.length > 0 ? "," : "")].join('') :
        ""),
      (props.length > 0 ? [lineEnd, props.join("," + lineEnd), lineEnd, indent.substr(tab.length)].join('') :
  ...
  var generateComplextContent = function(object, indent, depth, idx = 0) {
    var res = "";
    var child, c = 0;
    while (child = object.children[c++]) {
      if (child.name == "extension") {
        if (child.$base) {
          ...
          if (typedObject.name == "complexType")
            ...
            res += generateSimpleType(typedObject, indent, depth);
          else if ((typedObject.name == "complexContent") && (idx < 10))
            res += generateComplextContent(typedObject, indent, depth, idx + 1);
          else if (object.name == "element")
            res += typedObject.$type;
          else
            res += typedObject.name;
